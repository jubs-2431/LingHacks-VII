...
function makeFrame(index: number) {
  ...
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 920 620">
    <defs>
      ...
      <linearGradient id="risk" x1="0" x2="1">
        <stop offset="0" stop-color="rgba(255,255,255,0.92)" />
        <stop offset="1" stop-color="rgba(136,210,255,0.72)" />
      </linearGradient>
      <filter id="blurGlow">
    ...
    <g opacity="${glow}" filter="url(#blurGlow)">
      <circle cx="460" cy="310" r="${150 + t * 145}" fill="rgba(110,190,255,0.10)"/>
      <circle cx="625" cy="220" r="${70 + t * 55}" fill="rgba(255,255,255,0.07)"/>
    </g>
    <g transform="translate(130 ${lift}) rotate(${tilt} 330 300)">
      ...
      <rect x="46" y="118" width="560" height="8" rx="4" fill="rgba(255,255,255,0.22)"/>
      <rect x="46" y="146" width="505" height="8" rx="4" fill="rgba(255,255,255,0.18)"/>
      <rect x="46" y="174" width="536" height="8" rx="4" fill="rgba(255,255,255,0.16)"/>
      ...
      <circle cx="560" cy="431" r="${plain * 18}" fill="none" stroke="rgba(255,255,255,${plain * 0.8})" stroke-width="2"/>
      <path d="M550 431 l7 7 l16 -18" fill="none" stroke="rgba(255,255,255,${plain})" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
      <rect x="34" y="${scan}" width="592" height="2" rx="1" fill="rgba(255,255,255,0.88)" opacity="${0.14 + t * 0.58}"/>
    </g>
    <g opacity="${shield}" transform="translate(690 120)">
      <rect x="0" y="0" width="104" height="104" rx="28" fill="rgba(255,255,255,0.08)" stroke="rgba(255,255,255,0.28)"/>
      <path d="M52 22 C72 34 80 32 80 32 v22 c0 25 -18 40 -28 46 C42 94 24 79 24 54 V32s9 2 28 -10z" fill="none" stroke="rgba(255,255,255,0.82)" stroke-width="4"/>
    </g>
  </svg>`;
...
export default function LandingPage() {
  ...
  return (
    <div ref={pageRef} className="relative min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <motion.video ref={videoRef} className="fixed inset-0 z-0 h-[130vh] w-full object-cover" muted playsInline preload="auto" style={{ y: bgY }}>
      ...
      <div className="fixed right-6 top-1/2 z-20 hidden -translate-y-1/2 flex-col items-center gap-3 md:flex">
        {navItems.map((item, index) => (
          <a key={item.href} href={item.href} onClick={(event) => handleAnchorClick(event, item.id)} className={`${styles.scrollDot} ${activeSection === index ? styles.scrollDotActive : ""}`} aria-label={`Scroll to ${item.label}`} />
        ))}
      </div>
      ...
      <nav className="fixed inset-x-0 top-0 z-30 mx-auto flex max-w-7xl flex-row items-center justify-between px-8 py-6">
        <a href="#origin" onClick={(event) => handleAnchorClick(event, "origin")} className="text-3xl tracking-tight text-foreground" style={{ fontFamily: "var(--font-display), serif" }}>
          ElderShield<sup className="text-xs">®</sup>
        </a>

        <div className="liquid-glass hidden items-center gap-2 rounded-full px-3 py-2 md:flex">
          {navItems.map((item, index) => (
            <a key={item.href} href={item.href} onClick={(event) => handleAnchorClick(event, item.id)} className={`rounded-full px-4 py-2 text-sm transition-colors hover:bg-white/10 hover:text-foreground ${activeSection === index ? "bg-white/10 text-foreground" : "text-muted-foreground"}`}>
              {item.label}
            </a>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3">
          <ElderModeToggle />
          <Link href="/analyze" className="liquid-glass rounded-full px-6 py-2.5 text-sm text-foreground transition-transform duration-150 hover:scale-[1.03]">Analyze Document</Link>
        </div>
      </nav>
      ...
      <main className="relative z-10">
        <section id="origin" ref={scrollyRef} className={`${styles.scrollyChapter} min-h-[360vh]`}>
          <div className={styles.stickyScene}>
            <motion.div className={styles.sequenceStage} style={{ scale: sequenceScale, opacity: sequenceOpacity }}>
              <img src={frames[frameIndex]} alt="Scroll-controlled ElderShield document analysis frame" className={styles.sequenceImage} draggable={false} />
              <div className={styles.frameReadout}>FRAME {String(frameIndex + 1).padStart(2, "0")} / {FRAME_COUNT}</div>
            </motion.div>

            <div className={styles.heroCopy}>
              <Reveal>
                <h1 className={`max-w-7xl font-normal leading-[0.95] tracking-[-2.46px] ${elderMode ? "text-6xl sm:text-7xl md:text-8xl" : "text-5xl sm:text-7xl md:text-8xl"}`} style={{ fontFamily: "var(--font-display), serif" }}>
                  Where <em className="not-italic text-muted-foreground">clarity</em> rises <em className="not-italic text-muted-foreground">through the fine print.</em>
                </h1>
              </Reveal>
              <Reveal className="mx-auto mt-8 max-w-2xl">
                <p className={`leading-relaxed text-muted-foreground ${elderMode ? "text-2xl" : "text-base sm:text-lg"}`}>
                  ElderShield turns complex legal documents into plain-language risk maps for seniors, families, and anyone who wants to understand what they are signing before it matters.
                </p>
              </Reveal>
              <Reveal className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
                <a href="#signal" onClick={(event) => handleAnchorClick(event, "signal")} className={`liquid-glass rounded-full text-foreground transition-transform duration-150 hover:scale-[1.03] ${elderMode ? "px-16 py-6 text-2xl" : "px-14 py-5 text-base"}`}>Enter the story</a>
                <span className="text-sm text-muted-foreground">Scroll drives a generated frame sequence · reverse by scrolling up</span>
              </Reveal>
            </div>
          </div>
          ...
          <div className={styles.chapterTextStack}>
            {chapters.map(([number, title, body]) => (
              <Reveal key={number} className={`${styles.chapterPanel}`}>
                <span>{number}</span>
                <h2>{title}</h2>
                <p>{body}</p>
              </Reveal>
        ...
        <section id="signal" className={`${styles.cinematicSection} mx-auto flex min-h-screen max-w-7xl items-center px-6 py-28`}>
          <div className="grid w-full grid-cols-1 gap-8 lg:grid-cols-[0.88fr_1.12fr]">
            <Reveal className={`${styles.editorPanel} ${styles.depthCard} p-8 md:p-10`}>
              <p className="text-sm uppercase tracking-[0.35em] text-muted-foreground">Signal</p>
              <h2 className="mt-5 max-w-xl font-serif text-5xl leading-none tracking-tight md:text-7xl">The interface slows the contract down.</h2>
              <p className="mt-6 max-w-xl text-lg leading-8 text-muted-foreground">Inspired by Apple-style product storytelling and Methods Reconstructor-style provenance, the UI shows where each answer came from instead of floating in generic AI cards.</p>
              <Link href="/analyze" className="liquid-glass mt-8 inline-flex rounded-full px-8 py-4 text-sm text-foreground transition-transform hover:scale-[1.03]">Open Analyzer</Link>
            </Reveal>
            <div className={styles.signalStack}>
              {chapters.map(([number, title, body]) => (
                <Reveal key={title} className={`${styles.traceStrip}`}>
                  <span>{number}</span>
        ...
        <section id="trace" className={`${styles.cinematicSection} mx-auto flex min-h-screen max-w-7xl items-center px-6 py-28`}>
          <div className="w-full">
            <Reveal className="max-w-3xl">
              <p className="text-sm uppercase tracking-[0.35em] text-muted-foreground">Provenance trace</p>
              <h2 className="mt-5 font-serif text-5xl leading-none md:text-7xl">Not boxes. A visible reasoning trail.</h2>
            </Reveal>
            <div className={styles.traceGrid}>
              {traceRows.map(([label, title, body], index) => (
                <Reveal key={label} className={styles.traceNode}>
        ...
        <section id="access" className={`${styles.cinematicSection} mx-auto flex min-h-screen max-w-7xl items-center px-6 py-28`}>
          <Reveal className={`${styles.editorPanel} ${styles.depthCard} mx-auto max-w-4xl p-8 text-center md:p-12`}>
            ...
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">Elder Mode increases text size and contrast, explanations stay short, and the final checklist is easy to print or read aloud.</p>
          </Reveal>
        </section>

        <section id="demo" className={`${styles.cinematicSection} flex min-h-screen items-center justify-center px-6 py-28 text-center`}>
          <Reveal className={`${styles.editorPanel} ${styles.depthCard} max-w-3xl p-8 md:p-12`}>
            ...
            <p className="mt-6 text-lg leading-8 text-muted-foreground">The landing page tells the story. The analyzer proves the workflow.</p>
            <Link href="/analyze" className="liquid-glass mt-8 inline-flex rounded-full px-10 py-4 text-foreground transition-transform hover:scale-[1.03]">Analyze Document</Link>
          </Reveal>
        </section>
      </main>
    </div>
