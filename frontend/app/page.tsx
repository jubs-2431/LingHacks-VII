...
const featureCards = [
  [
    "Clause lens",
    "ElderShield scans the document for phrases that usually hide risk: arbitration, class-action waivers, automatic renewal, late fees, cancellation windows, and vague obligations. The goal is not to replace the document, but to slow down the exact language that deserves attention.",
  ],
  [
    "Source trace",
    "Every explanation stays connected to the original sentence. Instead of giving a floating AI summary, the interface shows what phrase triggered the warning, why it matters, and where the user can verify it in the source text.",
  ],
  [
    "Action checklist",
    "The final layer turns confusing clauses into practical next steps: what to ask, what to confirm, what dates to watch, and when to get help from a family member, caregiver, administrator, or lawyer before signing.",
  ],
];
...
function makeFrame(index: number) {
  ...
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 920 620">
    <defs><linearGradient id="card" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="rgba(255,255,255,0.22)"/><stop offset="1" stop-color="rgba(255,255,255,0.045)"/></linearGradient><filter id="blurGlow"><feGaussianBlur stdDeviation="18" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
    ...
    <g transform="translate(130 ${lift}) rotate(${tilt} 330 300)"><rect x="0" y="30" width="660" height="480" rx="34" fill="url(#card)" stroke="rgba(255,255,255,0.28)" stroke-width="1.4"/><rect x="46" y="82" width="230" height="12" rx="6" fill="rgba(255,255,255,0.72)"/><rect x="46" y="118" width="560" height="8" rx="4" fill="rgba(255,255,255,0.22)"/><rect x="46" y="146" width="505" height="8" rx="4" fill="rgba(255,255,255,0.18)"/><rect x="46" y="174" width="536" height="8" rx="4" fill="rgba(255,255,255,0.16)"/><rect x="46" y="228" width="${220 + riskA * 250}" height="34" rx="17" fill="rgba(255,255,255,${0.08 + riskA * 0.34})" stroke="rgba(255,255,255,${0.08 + riskA * 0.36})"/><rect x="46" y="286" width="${180 + riskB * 310}" height="34" rx="17" fill="rgba(136,210,255,${riskB * 0.25})" stroke="rgba(136,210,255,${riskB * 0.55})"/><rect x="46" y="374" width="560" height="1" fill="rgba(255,255,255,0.14)"/><rect x="46" y="410" width="${160 + plain * 420}" height="42" rx="21" fill="rgba(255,255,255,${plain * 0.16})" stroke="rgba(255,255,255,${plain * 0.32})"/><circle cx="560" cy="431" r="${plain * 18}" fill="none" stroke="rgba(255,255,255,${plain * 0.8})" stroke-width="2"/><path d="M550 431 l7 7 l16 -18" fill="none" stroke="rgba(255,255,255,${plain})" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><rect x="34" y="${scan}" width="592" height="2" rx="1" fill="rgba(255,255,255,0.88)" opacity="${0.14 + t * 0.58}"/></g>
...
export default function LandingPage() {
  ...
  const horizontalX = useTransform(featureProgress, [0, 0.14, 0.86, 1], ["0vw", "0vw", "-70vw", "-70vw"]);
  ...
  useMotionValueEvent(scrollyProgress, "change", (latest) => setFrameIndex(Math.max(0, Math.min(FRAME_COUNT - 1, Math.round(latest * (FRAME_COUNT - 1))))));
  ...
  return (
    <div ref={pageRef} className="relative min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <nav className="fixed inset-x-0 top-0 z-30 mx-auto flex max-w-7xl flex-row items-center justify-between px-8 py-6"><a href="#origin" onClick={(event) => handleAnchorClick(event, "origin")} className="text-3xl tracking-tight text-foreground" style={{ fontFamily: "var(--font-display), serif" }}>ElderShield<sup className="text-xs">®</sup></a><div className="liquid-glass hidden items-center gap-2 rounded-full px-3 py-2 md:flex">{navItems.map((item, index) => <a key={item.href} href={item.href} onClick={(event) => handleAnchorClick(event, item.id)} className={`rounded-full px-4 py-2 text-sm transition-colors hover:bg-white/10 hover:text-foreground ${activeSection === index ? "bg-white/10 text-foreground" : "text-muted-foreground"}`}>{item.label}</a>)}</div><div className="flex items-center gap-3"><ElderModeToggle /><Link href="/analyze" className="liquid-glass rounded-full px-6 py-2.5 text-sm text-foreground transition-transform duration-150 hover:scale-[1.03]">Analyze Document</Link></div></nav>
      <main className="relative z-10">
        <section id="origin" ref={scrollyRef} className={`${styles.scrollyChapter} min-h-[360vh]`}><div className={styles.stickyScene}><motion.div className={styles.sequenceStage} style={{ scale: sequenceScale, opacity: sequenceOpacity }}><img src={frames[frameIndex]} alt="Scroll-controlled ElderShield document analysis frame" className={styles.sequenceImage} draggable={false} /><div className={styles.frameReadout}>FRAME {String(frameIndex + 1).padStart(2, "0")} / {FRAME_COUNT}</div></motion.div><div className={styles.heroCopy}><Reveal><h1 className={`max-w-7xl font-normal leading-[0.95] tracking-[-2.46px] ${elderMode ? "text-6xl sm:text-7xl md:text-8xl" : "text-5xl sm:text-7xl md:text-8xl"}`} style={{ fontFamily: "var(--font-display), serif" }}>Where <em className="not-italic text-muted-foreground">clarity</em> rises <em className="not-italic text-muted-foreground">through the fine print.</em></h1></Reveal><Reveal className="mx-auto mt-8 max-w-2xl"><p className={`leading-relaxed text-muted-foreground ${elderMode ? "text-2xl" : "text-base sm:text-lg"}`}>ElderShield turns complex legal documents into plain-language risk maps for seniors, families, and anyone who wants to understand what they are signing before it matters.</p></Reveal><Reveal className="mt-12 flex justify-center"><a href="#signal" onClick={(event) => handleAnchorClick(event, "signal")} className={`liquid-glass rounded-full text-foreground transition-transform duration-150 hover:scale-[1.03] ${elderMode ? "px-16 py-6 text-2xl" : "px-14 py-5 text-base"}`}>Enter the story</a></Reveal></div></div><div className={styles.chapterTextStack}>{chapters.map(([number, title, body]) => <Reveal key={number} className={styles.chapterPanel}><span>{number}</span><h2>{title}</h2><p>{body}</p></Reveal>)}</div></section>
        ...
        <section id="features" ref={featuresRef} className={styles.horizontalSection}><div className={styles.horizontalSticky}><div className={styles.horizontalIntro}><p className={styles.featureEyebrow}>Core features</p><h2 className="mt-5 font-serif text-5xl leading-none md:text-7xl">Slide through the system.</h2><p className={styles.featureSubcopy}>This section pins in place: scroll down to move sideways through the three cards, then keep scrolling to continue down the page.</p></div><motion.div className={styles.horizontalTrack} style={{ x: horizontalX }}>{featureCards.map(([title, body], index) => <article key={title} className={styles.featureCard}><span>0{index + 1}</span><h3>{title}</h3><p>{body}</p></article>)}</motion.div></div></section>
